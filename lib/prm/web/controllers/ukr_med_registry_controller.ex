defmodule PRM.Web.UkrMedRegistryController do
  ...
  def create(conn, ukr_med_params) do
    with {:ok, %UkrMedRegistry{} = ukr_med_registry} <- Registries.create_ukr_med(
      ukr_med_params, get_consumer_id(conn)) do
  ...
  def update(conn, %{"id" => id} = ukr_med_params) do
    ...
    with {:ok, %UkrMedRegistry{} = ukr_med_registry} <- Registries.update_ukr_med(ukr_med_registry, ukr_med_params,
      get_consumer_id(conn)) do
